local function lock_group_tag(msg, data, target)
 if not is_momod(msg) then
  ...
  if group_tag_lock == 'yes' then
    return reply_msg(msg.id,'قفل #تـگ فعال میباشد !\n________\nهر گونه #تـگ(# - @) ارسالی توسط کاربران پاک خواهد شد !',ok_cb,false)
  else
    data[tostring(target)]['settings']['lock_tag'] = 'yes'
    save_data(_config.moderation.data, data)
    return reply_msg(msg.id,'قفل #تـگ فعال شد !\n________\nاکنون ، هر گونه #تـگ (# - @)ارسالی توسط کاربران حذف خواهد شد !',ok_cb,false)
  end
end
...
local function unlock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'no' then
    return reply_msg(msg.id,'قفل #تـگ غیرفعال میباشد!',ok_cb,false)
  else
    data[tostring(target)]['settings']['lock_tag'] = 'no'
    save_data(_config.moderation.data, data)
    return reply_msg(msg.id,'قفل #تـگ غیرفعال شد !',ok_cb,false)
  end
end
----------------------------------
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return reply_msg(msg.id,"قفل #عربی و #فارسی نویسی در گروه مجاز میباشد",ok_cb,false)
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return reply_msg(msg.id,"قفل #عربی و #فارسی نویسی در گروه مجاز شد !",ok_cb,false)
  end
end
---------------------------------------------------
...
function run(msg,matches)
		...
		if matches[1] == 'قفل' or matches[1]:lower() == 'lock' then
			...
			if matches[2] == 'تگ' or matches[2]:lower() == 'tag' or matches[2]:lower() == 'tag' then
