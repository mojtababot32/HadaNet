...
if msg.to.type == "channel" then
	...
	if (matches[1] == 'لغو نصب' or matches[1]:lower() == "rem") and not matches[2] and is_sudo(msg) then
		...
		mute_hash = 'hadanet:cli:mute_user:'..msg.to.id
		ban_hash = "hadanet:cli:gpban:"..msg.to.id
	...
	if (matches[1] == 'پاکسازی' or matches[1]:lower() == "clean") and is_owner(msg) then
			...
			if (matches[2] == "لیست مسدود" or matches[2]:lower() == "banlist") and is_owner(msg) then
				hash = 'hadanet:cli:gpban:'..msg.to.id
				redis:del(hash)
			...
			if (matches[2]:lower() == "لیست گلوبال بن" or matches[2]:lower() == "gbanlist") and is_sudo(msg) then
				local hash = 'hadanet:cli:gbanned'
				redis:del(hash)
			...
			if (matches[2] == 'لیست سکوت' or matches[2]:lower() == "silentlist") then
				chat_id = msg.to.id
				local hash =  'hadanet:cli:mute_user:'..chat_id
				redis:del(hash)
		...
		-- setowner
		if (matches[1] == "مالک گروه" or matches[1]:lower() == "setowner") and is_sudo(msg) then
		...
		if (matches[1]:lower() == "مقام") then
			...
			elseif is_owner(msg) then
				txt = "مقام شما : مدیر اصلی ربات در قبرستون\nستاره : ⭐️⭐️️"
			
			elseif is_momod(msg) then
				txt = "مقام شما : چس مدیر فرعی ربات در قبرستون\nستاره : ⭐️"
			
			else
				txt = "مقام شما : چس ممبر در قبرستون"
			end
